...
* 3/25 more on vector space V. defining L = linearly independent set of vectors; B = basis; S = set of vectors that collectively span V.
* 3/27 found visual proofs for Theorems 3.22 and 3.24. Big V to Small W *cannot* be injective (infinitely long line of other alternatives that the same T maps to). And 3.24 small V map to Large W *cannot* be surjective (b/c each point in the range is actually just one point in a 2-d infinte plane for the co-domain. So range << much smaller than Co-Domain W and therefore not surjective. Surjective requires range = Co-Domain.
